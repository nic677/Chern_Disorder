...
void Couple_matrix(MatrixXcf& Cqqprime, Vector2cf q, Vector2cf qPrime, MatrixXcf vecs, int size_sys, ArrayXi x, ArrayXi y);
...
Couple_matrix(C01, q0, q1,es.eigenvectors(),N,x,y);
Couple_matrix(C12, q1, q2,es.eigenvectors(),N,x,y);
Couple_matrix(C23, q2, q3,es.eigenvectors(),N,x,y);
Couple_matrix(C30, q3, q0,es.eigenvectors(),N,x,y);
...
///                    complex <float> qqpdot = (q(0)-qPrime(0))*(float)x + (q(1)-qPrime(1))*(float)y;
                Cqqprime(m,n)=((vecs.conjugate().col(m)(seq(size_sys*size_sys,2*size_sys*size_sys-1)).array()*vecs.col(n)(seq(size_sys*size_sys,2*size_sys*size_sys-1)).array() + vecs.conjugate().col(m)(seq(0,size_sys*size_sys-1)).array()*vecs.col(n)(seq(0,size_sys*size_sys-1)).array())*exp((complex<float>)1.0i*(complex<float>)(q(0)-qPrime(0))*x.cast<complex<float>>() + (complex<float>)1.0i*(complex<float>)(q(1)-qPrime(1))*y.cast<complex<float>>())).sum();
